...
local URL="${1}"
local name=`date +%s`
test "${target_dir}" = "" && target_dir="`pwd`/download"
mkdir -p "${target_dir}"
...
local output_folder="`pwd`/output/css_folder"
...
local output_folder="`pwd`/output/domain_folder"
mkdir -p "${output_folder}"
...
test -d "`pwd`/output" && rm -rf "`pwd`/output"
test -f "`pwd`/Marked_Ads.db" && rm -rf "`pwd`/Marked_Ads.db"
}
